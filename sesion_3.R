...
plot(m182_friend, layout=friend_layout, edge.arrow.size=.1)

social_layout <- layout.fruchterman.reingold(m182_social)
plot(m182_social, layout=social_layout, edge.arrow.size=.1)

task_layout <- layout.fruchterman.reingold(m182_task)
plot(m182_task, layout=task_layout, edge.arrow.size=.1)
...
m182_friend_no_iso <- set.vertex.attribute(m182_friend_no_iso,
                                          "color",
                                          value=friend_comm_wt$membership)

plot(m182_friend_no_iso,layout=layout.fruchterman.reingold(m182_friend_no_iso))
...
m182_friend_no_iso <- set.vertex.attribute(m182_friend_no_iso,
                                          "color",
                                          value=friend_comm_eb$membership)

plot(m182_friend_no_iso,layout=layout.kamada.kawai(m182_friend_no_iso,
                                                   niter=1000))
...
for (i in 0:(num_vertices-1)) {
	...
	if (num_comms > 1 & num_comms < num_vertices) {
		ideal_observed_cors <- append(ideal_observed_cors, 
		                              (gcor(idealized_comm_mat, as.matrix(friend_adj_mat_no_iso))))
...
friend_adj_mat_full <- get.adjacency(m182_friend)
which(igraph::degree(graph.adjacency(friend_adj_mat_full))==0)
...
friend_blockmodel <- blockmodel(as.matrix(friend_adj_mat_full), communities_full)
friend_blockmodel
...
social_adj_mat
social_blockmodel <- blockmodel(as.matrix(social_adj_mat), communities_full)
social_blockmodel
...
# overall mean.
social_mean <- mean(as.matrix(social_adj_mat))
social_mean
...
task_adj_mat
task_blockmodel <- blockmodel(as.matrix(task_adj_mat), communities_full)
task_blockmodel

task_mean <- mean(as.matrix(task_adj_mat))
task_mean
...
g1 <- graph.ring(10)
g1 <- add.edges(g1, c(0,2, 1,3, 0,3, 5,7, 5,8, 6,8)+1)
make_k_core_plot(g1)
